...
-- 데이터 삭제시 삭제할 데이터를 참조하는 처리를 어떻게 할 것인가?
-- spring_board bno 가 삭제가 될 떄 게시물에 달려있는 댓글은 어떻게 할것인가 ?


-- 우리는  spring_board bno 가 삭제가 될 떄 첨부파일은 그냥 삭제를 해도 됐다. 근데 왜 reply객체는 삭제를 안했냐 ?
--  사이트의 규칙에 따라서 달라질 수 있다.
-- 댓글 테이블 만들기 
create table spring_reply(
	rno number(10,0) constraint pk_reply primary key,						-- 댓글 글번호
	bno number(10,0) not null,												-- 원본 글번호
	...
	updatedate date default sysdate,										-- 댓글 수정일
	constraint fk_reply_board foreign key(bno) references spring_board(bno) -- 외래키 설정
	ON DELETE CASCADE  -- 부모의 bno가 삭제되면 같이 삭제되도록 만든다... 
);
