...
//	<bean id="customUserDetailService" class="com.company.service.CustomUserDetailService"></bean>
	...
	@Bean
	public CustomUserDetailService getUserDtailService() {
...
//	<bean id="customAccessDeniedHandler" class="com.company.handler.CustomAccessDeniedHandler">	</bean>
	...
	@Bean
	public AccessDeniedHandler getaAccessDeniedHandler() {
...
//	<bean id="customLoginSuccessHandler" class="com.company.handler.CustomLoginSuccessHandler"></bean>
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.exceptionHandling(ex -> ex.accessDeniedHandler(getaAccessDeniedHandler()));
		
//		<security:form-login login-page="/login" 
		http.formLogin().loginPage("/login").failureUrl("/login-error").successHandler(getAuthenticationSuccessHandler());
		// <security:logout ~
		http.logout().logoutUrl("/logout").invalidateHttpSession(true).deleteCookies("remember-me","JSESSION_ID").logoutSuccessUrl("/");
		
		http.authorizeRequests().antMatchers("/user-page").access("hasRole('ROLE_SER')")
...
//		</security:authentication-manager>
		auth.userDetailsService(getUserDtailService()).passwordEncoder(getPasswordEncoder());
